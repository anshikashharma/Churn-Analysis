...
Sum(Total_Revenue)*100/(select sum(Total_Revenue) from stg_churn) as rev_percent
from stg_churn
group by Customer_Status;

select State,Count(State) as Total_Count,
count(State)*100.0/(select count(*) from stg_churn) as percent_state
from stg_churn
group by State
order by percent_state DESC;
SELECT distinct(Internet_Type) from stg_churn;

-- TO FIND THE NULL COUNT IN EVERY COLUMN 
SELECT 
